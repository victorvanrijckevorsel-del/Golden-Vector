...
- validation failures: retain current status code, message, entered values, and redirect/render target. Audit correction: the four ticker-detail validation branches (`workspace.py:615/652/703/735`) currently **crash to the generic 500 page whenever the ticker has Tool A data** (the error-branch re-render passes `app_config=None`, so `build_delta_explanation` raises); only without Tool A artifacts does the intended 400 page render, and that page loses all lens/window/source/calculator/query context, hardcodes a `12M` canonical anchor, styles the error with the success `flash` class, and drops column help. An all-blank company POST also redirects to a `Company inputs saved.` flash without saving anything. All of this is defect register D1/D2 — pre-existing behaviour, characterized by `tests/test_redesign_routes.py` and preserved during the redesign, then fixed in the post-Phase-8 defect follow-up, never opportunistically during markup migration.
...
Pre-existing behaviour issue to keep separate: the four ticker-form validation-error branches re-render the default detail state without carrying any active lens/window/source/calculator argument — and because they pass `app_config=None`, the re-render **crashes to the generic 500 page whenever the ticker has Tool A data**, which is every real ticker on the live workspace. When the 400 page does render (Tool A artifacts absent), it marks the wrong canonical anchor (`12M` hardcoded), presents the error in the success flash style, and drops column help; the collapsed `return_to` compounds the loss on the next successful save. This is defect register D1: characterized during the redesign, fixed in the post-Phase-8 defect follow-up, never silently changed inside markup migration.
