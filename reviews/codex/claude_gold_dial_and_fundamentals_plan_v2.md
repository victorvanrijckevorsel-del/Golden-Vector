...
- Raw Yahoo statements are a durable backend data asset, not temporary receipts. PR5 stores them under `data/raw/fundamentals/` as immutable run-stamped artifacts plus a latest alias and a small fetch manifest (see §5h.1). They must be readable through a shared backend reader before any mapper/UI consumes them.
...
2. **Raw Yahoo storage (`fundamentals/raw_store.py`, new).** Before mapping, persist the fetched Yahoo statements as the backend source-of-truth:
   - Write `raw_fundamentals_statements_latest_{source_run_id}.parquet` plus `raw_fundamentals_statements_latest.parquet` under `data/raw/fundamentals/`, using the same atomic write and run-id identity rules as `fetched_fundamentals`.
   - Store a queryable long table, not pickles: `schema_version`, `ticker`, `yahoo_symbol`, `statement_type` (`income_stmt|balance_sheet|cashflow`), `line_item_original`, `period_end`, `value_raw`, `financial_currency`, `fetched_at_utc`, `source_run_id`, `fetch_status`, `error_message`. This is "raw" in the important sense: no alias mapping, no currency conversion, no scale-to-millions, no financial judgment. It is only reshaped so we can query it later.
   - Write `latest_fundamentals_fetch_manifest.json` with `source_run_id`, `fetched_at_utc`, raw artifact paths/hashes, per-ticker status, statement row counts, and fetch/map timings. This is the pointer for raw fundamentals history; the model-state manifest still points at the mapped official artifact.
   - Add shared readers: `load_raw_fundamentals_statements(paths, source_run_id=None, ticker=None, statement_type=None)` and `load_latest_fundamentals_fetch_manifest(paths)`. Future mapper reruns and PR6 comparisons must call these readers rather than scanning directories or touching Yahoo.
   - Retention rule: raw fundamentals artifacts referenced by any retained fundamentals fetch manifest are protected from pruning. Default behavior keeps the raw archive; any future pruning must be dry-run by default and prove it never deletes referenced raw statements.
3. **Consumer/mapper (`fetch_fundamentals.py`, new).** Takes an **injected** `YahooClient` (mirror `fetch_options.py:51-58`), never constructs one. The safe sequence is: (a) fetch per ticker with isolation; (b) persist the raw statements through `fundamentals/raw_store.py`; (c) map from the persisted raw artifact, not from the in-memory fetch result; (d) write the resolved official rows through `write_fetched_fundamentals_artifact_pair(...)` (§5b). **`layer1.py`/`layer2.py` must never import `Ticker.*`** — a guardrail test asserts the model reads only the persisted canonical table.
...
**PR5 — Yahoo statement ingestion + canonical mapper.** **Built to the FULL SPEC in §5h** (the load-bearing component), with the raw Yahoo archive treated as the product's backend data center for official fundamentals. Build order:
  1. **Mapper-first, no Yahoo:** add fixtures + raw-store reader/writer + canonical mapper tests. The mapper reads the persisted raw long table, never in-memory Yahoo frames.
  2. **Fetch command second:** add thin `YahooClient.fetch_financial_statements` (returns `{}` on failure) + `fetch_fundamentals.py` orchestration that stores raw statements first, then maps from storage, then writes `fetched_fundamentals`.
  3. **Publish/freshness last:** add the raw fetch manifest, official artifact manifest entry update, central freshness line, and pruning protection.
In brief: raw Yahoo statements are persisted first and kept; mapping then uses the §5h.2 alias tables, normalizes in the §5h.3 order (currency→USD via `merge_fx_asof`, then `÷1e6`, kept separate from the GBp price scale), applies the §5h.4 status tree + §5h.5 reconciliation gate, and writes `fetched_fundamentals` through the PR4 writer. `layer1/layer2` never import `Ticker.*`.
- `[v2.1 — Codex 2nd-pass HIGH B, adopted]` **The refresh/publish contract, explicit:**
  ...
  - Outputs: run-stamped raw Yahoo statements + latest alias + `latest_fundamentals_fetch_manifest.json` under `data/raw/fundamentals/`, then the run-stamped official artifact + latest alias + the `fundamentals_official` manifest entry carrying `source_run_id` and `fetched_at_utc`.
...
- **Gate:** the §5h.7 fixture suite (US/London/TSX/ASX + missing-leg + divergent-EBITDA + negative-pretax), all no-live-call; raw-store round-trip preserves original line labels/periods/currencies/values exactly; mapper reads from persisted raw storage, not from the fetch result; missing-leg→`MISSING`-not-0; pence-vs-`/1e6` non-conflation; per-item degrade never aborts the pull; **stage summary contains per-step seconds + rows; raw fetch manifest written; official manifest entry written; freshness line fires on an artificially old `fetched_at_utc`; pruning test proves retained raw fundamentals artifacts are not deleted.**
