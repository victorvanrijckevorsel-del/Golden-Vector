...
## Codex Phases 3–4 review 2026-08-10 — GV-RD-P34-1…6 (ALL RESOLVED same day on dev-vic)

Review record: `reviews/codex/2026-08-10_visual_redesign_phases_3_4_review.md`.

- **P34-1 (P1) — anonymous containment fallback / focus defects: RESOLVED.** All 10 remaining detail-page tables (8 in `detail_panels.py`, 2 in `detail_forms.py`) wrapped in labelled `table_region`s (Opus worker, ids `detail-*-table-region`); panel/`two-column` rules keep only `min-width:0` (no overflow-x); a design-token scan now forbids `overflow-x:auto` outside `.table-region` (pre-existing `.candidate-top-list-card` scroll removed); shell JS drops `tabindex` on non-overflowing regions and restores it on resize (server-rendered tabindex stays the no-JS default). Verified live: /ticker/NEM 0px overflow at 4 widths with the fallback gone; 4 regions → 3 scrollable → exactly 3 tab stops; ArrowRight scrolls the focused verification region (`browser_evidence_p34_fixes.json`).
- **P34-2 (P1) — Lab STALE rendered danger: RESOLVED.** `STALE`/`CELLS_STALE`/`CELLS_EMPTY` now warning per §10.5; corrupt/unreadable/missing artifacts stay danger; UNKNOWN_SCENARIO keeps no-rebuild wording. 17 parameterized state→tone tests at both real render call sites.
- **P34-3 (P2) — header scopes: RESOLVED.** `help_th` emits `scope="col"` by default with an explicit `scope="row"` argument (validated); 5 k/v call sites row-scoped; hand-written headers scoped across CF builder, Option Trading liquidity, Tool D, detail forms; Portfolio correlation matrix has col-scoped header row + row-scoped body headers; the option-candidates group header is `scope="colgroup"`. 9 datatables assertions updated (not weakened) to the new markup.
- **P34-4 (P2) — trust boundary: RESOLVED.** `notice(extra_classes=…)` validates CSS class tokens and raises on anything else; `status_strip` names its trusted fragment `value_html` and escapes labels; `table_region` validates its id token and requires a non-empty label. Hostile-input tests added.
- **P34-5 (P2) — shallow component tests: RESOLVED.** Added: status-strip contract, extra-classes safety, region id/label validation, state→tone call-site tests, region focusability in the Node shim (conditional tab stops incl. resize flip), and an AST purity test pinning `serve/ui/` to stdlib-only imports.
- **P34-6 (P3) — dropped copy: RESOLVED.** Missing-manifest branch renders "**Latest Market Snapshot.** No validated local market-data snapshot is available yet." visibly in the neutral notice; direct render assertions cover both branches.
