...
**2026-06-12 fix pass:** all HIGH/MED/LOW + N1/N2/N3/N5 fixed across four commits (Batches A+B at 55d1be4, C+D following). Open: N4 (iv_rank column+hint), N6 (tooltip row-horizon), the serve-purity verifier coverage gap, and the recorded realized-vol calendar-vs-trading basis note.

## HIGH (ship blockers)

- [x] **H1 — Skew-curve chart structurally empty for 3 of 4 new horizons** (found independently
...
- [x] **H2 — Legacy history backfill manufactures rows (`nan is not None`)** (contract 2).
...
- [x] **M1 — LIMITED_HISTORY gate counts rows, not usable observations** (contracts 1+2 agree).
...
- [x] **M2 — Validator gaps (3 closely related)** (contract 1):
...
- [x] **M3 — Fresh-publish freshness domain ignores schema version** (contract 5; the one
...
- [x] **M4 — C4 most-liquid override unguarded against non-displayable stamps** (contract 3).
...
- [x] **M5 — Stamped `most_liquid_*_expiration` has zero consumers** (contract 3). Persisted and
...
- [x] **M6 — 550d realized vol is silently full-history vol** (contract 4). `_realized_vol`
...
- [x] **M7 — Orphaned dead code from Milestone B: `tier_count` body unreachable inside
...
- [x] **M8 — Group-vote recomputes every per-ticker selection** (contract 3). `_stamp_most_liquid_defaults`
...
- [x] **L1 — `OptionSizingRequest` default horizon is the retired 60** (contracts 1+3+4 agree).
...
- [x] **L2 — `signal_horizon_from_row` fallback is literal 60.** Should fall back to None (let
  the chart's documented min-of-points fallback run) rather than a stale constant.
  (`option_signal_render.py:23-33`)
- [x] **L3 — Stale "Invalid horizon; defaulted to 90d" note when the most-liquid override then
  applies** (e.g. old `?horizon=60` bookmark → note says 90d, page shows 230d). **Fix:** append
  the resolved value after the override.
- [x] **L4 — `build_option_trading_detail(target_horizons_days=...)` is a no-op param** — serve
  threads config into a parameter the body never reads. Fixed together with L1/M4.
- [x] **L5 — Signal-area constants vs configurable signal horizon**: validator accepts a signal
...
- [x] N1 — Cross-window tie-break drops `is_standard_monthly` (and documents dropping
  DTE-distance but not that one). Add to the key + docstring. (`option_horizon_selection.py:190-201`)
- [x] N2 — Schema comparator parity: `_schema_version_matches` accepts "3.9", serve gate is
  strict string-eq. Tighten to normalized string equality. (`model_state.py:1167-1174`)
- [x] N3 — UNAVAILABLE reason degrades on rebound publishes (stub entries re-derive a generic
...
- [x] N5 — `candidate_puts` keeps (30,60,90) literal defaults; config-model defaults still
