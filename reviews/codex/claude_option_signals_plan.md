...
## 5b. Data timing — auto-refresh during market hours + stale-quote guard (Emanuel)
**Lesson from the GDX/GDXJ incident:** option signals are only valid when the **US options market is open** (live two-sided quotes). A pre-market/closed-market refresh gets empty bid/ask → nothing tradable → silently wrong. The stock side is immune (it uses the persistent daily *close*); the option side needs **live** quotes. Two fixes:

**1) Auto-refresh on a repeatable schedule, during market hours.** The tool refreshes itself **once or twice every US trading day at fixed times while the options market is open** (e.g. ~11:00 and ~14:30 ET / ~17:00 and ~20:30 Emanuel-local), not at an arbitrary hour. Consistent timing isn't just convenience — the **OI-change and skew-evolution signals compare consecutive snapshots, so they're only meaningful if each snapshot is taken at a comparable, market-open time** (a pre-market snapshot vs a mid-session one makes OI-change garbage). Implementation: a scheduled task (Windows Task Scheduler) running `python main.py refresh` at the fixed times on **US trading days only** (skip weekends/market holidays); the existing refresh lock prevents overlaps. Keep it simple and external (a documented scheduled task + a small setup helper), not a bespoke long-running daemon.

**2) Stale-quote guard (no silent failure, no clobbering good data).** Every refresh measures option-quote freshness = the fraction of chain contracts with **both** a live bid and ask (≈85% during market hours vs ≈8% pre-market). If coverage is below a threshold (per chain):
- **Do not overwrite** the last good option candidates/signals with the empty ones — **keep the previous live snapshot.**
- **Say so plainly** in the UI: *"Option quotes are stale/pre-market (only X% two-sided) — showing the last live snapshot from [time]; refresh during US market hours."*
- Per-chain, so one bad name doesn't blank the universe.

This makes a mistimed run **harmless instead of destructive** — and is exactly the no-silent-failure pattern we used for the Tool B stale-schema guard. (The §3 confidence gate judges *one contract's* tradability; this guard judges *the whole refresh's* freshness and protects the published data.)
...
## 10. Self-review
Grounded in our own peer-reviewed research (two strong signals — skew *and* OI-change/flow; expensiveness via IV-vs-RV/IV-rank; static put/call *level* honestly down-ranked) and in the existing code (extends `features/options.py`, reuses `liquidity_tier`, reads the existing archive). It is symmetric, backend-only, blunt (states the truth, no hedging), covers GDX/GDXJ as a sector reference, includes the three charts Emanuel asked for (data computed at refresh, UI only plots), and avoids a black-box composite. It also fixes the **data-timing fragility** the GDX/GDXJ incident exposed: a market-hours auto-refresh (consistent snapshots, which the OI/skew-evolution signals *require*) plus a stale-quote guard that protects published data instead of silently wiping it. Main risks: (a) Yahoo IV/OI data quality → the per-contract confidence gate *and* the per-refresh stale-quote guard are both essential, and OI change needs two clean consecutive market-hours snapshots; (b) history/flow thin at launch → label it and **protect option snapshots from pruning** so the archive accumulates; (c) keep the static put/call ratio honestly down-ranked while the OI *change* carries the positioning signal. All §9 decisions resolved.

## 11. What we explicitly want Codex to challenge (review brief)
This plan is a starting point, not the final word — push on it:
- **Better interpretation of the numbers.** Is `skew + OI-flow + IV-rank` the best way to turn the chain into a *strong, decision-useful* signal, or is there a sharper reading (e.g., a different skew measure like 25Δ risk-reversal vs put−call IV; combining the name's skew with the GDX/GDXJ sector skew into a cleaner "idiosyncratic downside" read; an IV-vs-realized term-structure view)? Propose what you'd actually trust and why.
- **Better, decision-helping charts.** The three proposed (skew curve, skew-over-time, OI-by-strike) are a first cut. What visualization would *most* help a non-expert decide — and make the tilt obvious at a glance? Suggest alternatives/additions (e.g., a single per-name "signal gauge," a sector-vs-name skew overlay, an IV-rank heatmap across the universe).
- **Anything wrong/risky** in the metric math, the symmetry, the stale-quote/scheduler design, or the honesty of the evidence grading. Grade READY / READY WITH MINOR CHANGES / NEEDS CHANGES and save findings in `reviews/codex/`.
