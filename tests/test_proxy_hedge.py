...
def test_map_proxy_hedges_orders_optionable_matches_by_down_beta_diff():
    tool_a = pd.DataFrame(
        [
            {"ticker": "TARGET", "down_beta_core": 1.40, "confidence_score": 0.80},
    ...
    assert matches[0].basis_risk_label == "low_basis_risk"
    assert matches[0].tool_a_confidence == pytest.approx(0.80)
    assert matches[0].tool_b_verdict == "WATCH"
    assert matches[1].basis_risk_label == "high_basis_risk"


def test_map_proxy_hedges_marks_medium_basis_risk_for_wider_confident_match():
    tool_a = pd.DataFrame(
        [
            {"ticker": "TARGET", "down_beta_core": 1.40, "confidence_score": 0.40},
            {"ticker": "AEM", "down_beta_core": 1.20, "confidence_score": 0.80},
        ]
    )

    mapping = map_proxy_hedges(
        non_optionable_tickers=["TARGET"],
        optionable_tickers=["AEM"],
        tool_a_frame=tool_a,
        top_n=1,
        max_beta_diff=0.35,
    )

    assert mapping["TARGET"][0].basis_risk_label == "medium_basis_risk"
