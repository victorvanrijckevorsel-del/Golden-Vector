...
def test_fmt_numeric_td_emits_raw_number_as_data_order():
    cell = _fmt_numeric_td(107.4, decimals=1, as_percent=True)
    assert cell == '<td data-order="107.4">10,740.0%</td>'.replace(
        # _fmt_numeric_td with as_percent multiplies by 100; double-check
        # display matches convention.
        "10,740.0", "10,740.0",
    ) or '<td data-order="107.4">' in cell
    # Tighter: data-order IS the raw 107.4; display is formatted percent.
    assert 'data-order="107.4"' in cell
